...
|------|-----|
| ページ種別 | リファレンス型 |
| アイテム数目安 | スラッシュコマンド約70件（9カテゴリ）、CLIコマンド約7件、CLIフラグ約7件、ショートカット約30件 |
| WebFetch 欠落リスク | 高 |

### 高リスクフィールド

（WebFetchのAI要約で特に欠落・不正確になりやすいフィールド）

- `args`: コマンドの引数。WebFetch の要約で省略されやすい
- `whenToUse`: 使用シーンの詳細。要約で簡略化されやすい
