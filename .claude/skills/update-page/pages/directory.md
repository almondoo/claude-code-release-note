...
|------|-----|
| ページ種別 | 構造型 |
| アイテム数目安 | 約30エントリ（5セクション）、優先順位5レベル |
| WebFetch 欠落リスク | 高（8つのソースURLに情報が分散） |

### 高リスクフィールド

- `vcs`: コミット推奨/ローカル専用/OS管理の区別。複数ドキュメント間で矛盾する可能性がある
- `detail`: 情報量が多く、WebFetch の要約で技術的背景が欠落しやすい
- `recommended`: 推奨レベルの判定。公式ドキュメントで明示されていない場合がある
