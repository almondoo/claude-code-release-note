...
|------|-----|
| ページ種別 | 履歴型 |
| アイテム数目安 | バージョン数は増加し続ける（既存データは不変、追加のみ） |
| WebFetch 欠落リスク | 中（CHANGELOG は長大だが、対象バージョンのセクションのみ抽出） |

### 高リスクフィールド

- `tags`: 変更タイプ（4種）と対象領域（9種）の分類。CHANGELOG の記述から適切なタグを判定する必要がある
- `detail`: 技術的背景の詳細。CHANGELOG の1行要約だけでは不十分で、Docs からの補完が必要
...
- Claude Code 以外のプロダクトのリリースノートには使用しない
- リリースノートの表示・UI の修正はこのスキルの対象外（コード変更が必要）
