...
- タグは2軸構造（全13種類）:
  ...
  - 対象領域 (9種): `SDK`, `IDE`, `Platform`, `Windows`, `Security`, `Perf`, `Plugin`, `MCP`, `Agent`
...
**ファイルサイズの制限**: なし。ファイルの分割は10刻みのバージョン範囲単位でのみ行い、サイズを理由とした分割はしない
...
- 独立したタスクは Agent ツールを使って並列実行し、効率的に実装する
- 複数ファイルの調査・編集・データ作成など依存関係のない作業は同時にエージェントを起動する

## 実装後チェック

- `pnpm run check` — 型チェック + ESLint + Prettier を一括実行。実装完了後に必ず実行すること
- ブラウザでの表示確認が必要な場合のみ `scripts/start-server.sh` を使用する
