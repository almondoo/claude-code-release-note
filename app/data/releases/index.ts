import releases20x from "~/data/releases/releases-2.0.x.json";
import releases21x0x from "~/data/releases/releases-2.1.0x.json";
import releases21x1x from "~/data/releases/releases-2.1.1x.json";
import releases21x2x from "~/data/releases/releases-2.1.2x.json";
import releases21x3x from "~/data/releases/releases-2.1.3x.json";
import releases21x4x from "~/data/releases/releases-2.1.4x.json";
import releases21x5x from "~/data/releases/releases-2.1.5x.json";
import releases21x6x from "~/data/releases/releases-2.1.6x.json";
import releases21x7x from "~/data/releases/releases-2.1.7x.json";
import releases21x8x from "~/data/releases/releases-2.1.8x.json";
import releases21x9x from "~/data/releases/releases-2.1.9x.json";
import releases21x10x from "~/data/releases/releases-2.1.10x.json";
import releases21x11x from "~/data/releases/releases-2.1.11x.json";
import releases21x12x from "~/data/releases/releases-2.1.12x.json";
import releases21x13x from "~/data/releases/releases-2.1.13x.json";
import releases21x14x from "~/data/releases/releases-2.1.14x.json";
import releases21x15x from "~/data/releases/releases-2.1.15x.json";
import releases21x16x from "~/data/releases/releases-2.1.16x.json";
import releases21x17x from "~/data/releases/releases-2.1.17x.json";
import releases21x18x from "~/data/releases/releases-2.1.18x.json";
import releases21x19x from "~/data/releases/releases-2.1.19x.json";
import releases21x20x from "~/data/releases/releases-2.1.20x.json";
import releases21x21x from "~/data/releases/releases-2.1.21x.json";
import releases21x22x from "~/data/releases/releases-2.1.22x.json";
import releases21x23x from "~/data/releases/releases-2.1.23x.json";
import releases21x24x from "~/data/releases/releases-2.1.24x.json";
import releases21x25x from "~/data/releases/releases-2.1.25x.json";
import releases21x26x from "~/data/releases/releases-2.1.26x.json";
import releases21x27x from "~/data/releases/releases-2.1.27x.json";
import versionDetails21x2x from "~/data/releases/version-details-2.1.2x.json";
import versionDetails21x3x from "~/data/releases/version-details-2.1.3x.json";
import versionDetails21x4x from "~/data/releases/version-details-2.1.4x.json";
import versionDetails21x5x from "~/data/releases/version-details-2.1.5x.json";
import versionDetails21x6x from "~/data/releases/version-details-2.1.6x.json";
import versionDetails21x7x from "~/data/releases/version-details-2.1.7x.json";
import versionDetails21x8x from "~/data/releases/version-details-2.1.8x.json";
import versionDetails21x9x from "~/data/releases/version-details-2.1.9x.json";
import versionDetails21x10x from "~/data/releases/version-details-2.1.10x.json";
import versionDetails21x11x from "~/data/releases/version-details-2.1.11x.json";
import versionDetails21x12x from "~/data/releases/version-details-2.1.12x.json";
import versionDetails21x13x from "~/data/releases/version-details-2.1.13x.json";
import versionDetails21x14x from "~/data/releases/version-details-2.1.14x.json";
import versionDetails21x15x from "~/data/releases/version-details-2.1.15x.json";
import versionDetails21x16x from "~/data/releases/version-details-2.1.16x.json";
import versionDetails21x17x from "~/data/releases/version-details-2.1.17x.json";
import versionDetails21x18x from "~/data/releases/version-details-2.1.18x.json";
import versionDetails21x19x from "~/data/releases/version-details-2.1.19x.json";
import versionDetails21x20x from "~/data/releases/version-details-2.1.20x.json";
import versionDetails21x21x from "~/data/releases/version-details-2.1.21x.json";
import versionDetails21x22x from "~/data/releases/version-details-2.1.22x.json";
import versionDetails21x23x from "~/data/releases/version-details-2.1.23x.json";
import versionDetails21x24x from "~/data/releases/version-details-2.1.24x.json";
import versionDetails21x25x from "~/data/releases/version-details-2.1.25x.json";
import versionDetails21x26x from "~/data/releases/version-details-2.1.26x.json";
import versionDetails21x27x from "~/data/releases/version-details-2.1.27x.json";

export const RELEASES = [
  ...releases20x,
  ...releases21x0x,
  ...releases21x1x,
  ...releases21x2x,
  ...releases21x3x,
  ...releases21x4x,
  ...releases21x5x,
  ...releases21x6x,
  ...releases21x7x,
  ...releases21x8x,
  ...releases21x9x,
  ...releases21x10x,
  ...releases21x11x,
  ...releases21x12x,
  ...releases21x13x,
  ...releases21x14x,
  ...releases21x15x,
  ...releases21x16x,
  ...releases21x17x,
  ...releases21x18x,
  ...releases21x19x,
  ...releases21x20x,
  ...releases21x21x,
  ...releases21x22x,
  ...releases21x23x,
  ...releases21x24x,
  ...releases21x25x,
  ...releases21x26x,
  ...releases21x27x,
].reverse();

export const VERSION_DETAILS = {
  ...versionDetails21x2x,
  ...versionDetails21x3x,
  ...versionDetails21x4x,
  ...versionDetails21x5x,
  ...versionDetails21x6x,
  ...versionDetails21x7x,
  ...versionDetails21x8x,
  ...versionDetails21x9x,
  ...versionDetails21x10x,
  ...versionDetails21x11x,
  ...versionDetails21x12x,
  ...versionDetails21x13x,
  ...versionDetails21x14x,
  ...versionDetails21x15x,
  ...versionDetails21x16x,
  ...versionDetails21x17x,
  ...versionDetails21x18x,
  ...versionDetails21x19x,
  ...versionDetails21x20x,
  ...versionDetails21x21x,
  ...versionDetails21x22x,
  ...versionDetails21x23x,
  ...versionDetails21x24x,
  ...versionDetails21x25x,
  ...versionDetails21x26x,
  ...versionDetails21x27x,
};
