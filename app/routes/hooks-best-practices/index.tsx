...
export const meta = (): Array<{ title?: string; name?: string; content?: string }> => {
  return [
    { title: "Claude Code Hooks ベストプラクティス" },
    { name: "description", content: "Hooks を効果的に使うためのベストプラクティスとレシピ集" },
  ];
};
...
const HooksBestPractices = (): React.JSX.Element => {
  const {
    query,
    setQuery,
    activeTab,
  ...
  return (
    <div className="min-h-screen bg-slate-900 font-sans text-slate-100">
      <div className="max-w-[1400px] mx-auto px-4 py-8">
        {/* Header */}
        <PageHeader
          title="Hooks ベストプラクティス"
          description="Claude Code のライフサイクルに Hooks を組み込み、自動フォーマット・セキュリティチェック・通知・監査ログを決定論的に実行するためのガイド。"
          stats={[
        ...
        <motion.div
          ...
          className="mb-4"
        >
          <SearchInput value={query} onChange={setQuery} placeholder="Hooks を検索..." />
        </motion.div>
...
export default HooksBestPractices;
